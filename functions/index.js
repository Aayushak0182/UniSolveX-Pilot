...
const STORE_REF = db.collection("workspace").doc("default")
const GEMINI_API_KEY = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY || ""
const ADMIN_EMAIL = normalizeEmail(process.env.ADMIN_EMAIL || "aayushak0182@gmail.com")
const DEFAULT_FIREBASE_CONFIG = {
  apiKey: process.env.FIREBASE_API_KEY || "AIzaSyBT5JHdKOanXqtyzLPnQfCk9zCpK2LmQWo",
  authDomain: process.env.FIREBASE_AUTH_DOMAIN || "unisolvex-pilot.firebaseapp.com",
  projectId: process.env.FIREBASE_PROJECT_ID || "unisolvex-pilot",
  storageBucket: process.env.FIREBASE_STORAGE_BUCKET || "unisolvex-pilot.firebasestorage.app",
  messagingSenderId: process.env.FIREBASE_MESSAGING_SENDER_ID || "927520165953",
  appId: process.env.FIREBASE_APP_ID || "1:927520165953:web:c49e000896ce269a5ef6c0",
  measurementId: process.env.FIREBASE_MEASUREMENT_ID || "G-N0DYG90N5H",
}
...
exports.helloWorld = onRequest((req, res) => {
  res.send("Hello from UniSolveX Pilot")
})
...
async function readStore() {
  const snapshot = await STORE_REF.get()
  const defaults = createDefaultStore()

  if (!snapshot.exists) {
    await STORE_REF.set(defaults)
    return defaults
  }
...
async function generateAiDraft(payload) {
  ...
  try {
    ...
    if (!response.ok || !rawText) {
      const apiError = payloadJson.error && payloadJson.error.message ? payloadJson.error.message : "AI generation failed"
